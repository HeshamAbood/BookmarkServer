...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_response(400,message="Some form elements are missing")
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Some form elements are missing!".encode())
            return
        ...
        if CheckURI(longuri):
            ...
            self.send_response(404,"the website not found") 
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(
